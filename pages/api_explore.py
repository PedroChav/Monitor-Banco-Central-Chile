...
@st.cache_data
def buscador_multiseries(id_serie:list,starts:str,ends:str,nombre=list):
    df_id = siete.cuadro(
        series = id_serie,
        desde = starts,
        hasta = ends,
...
with st.expander("Visualizador de series"):
    ...
    if st.session_state.seriesid not in (None, ""):
        df_seriesid = buscador_seriesid(id_serie = st.session_state.seriesid,
                                        start=st.session_state.start,
                                        end=st.session_state.end,
                                        nombre=st.session_state.serie_name if st.session_state.serie_name not in (None, "") else "empty"
                                        )
        tab1, tab2 = st.tabs(["Grafico", "Data"])
        tab1.line_chart(df_seriesid)
        tab2.dataframe(df_seriesid)
    else:
        tab1, tab2 = st.tabs(["Grafico", "Data"])
        tab1.text("Empty")
        tab2.dataframe()
...
with st.expander("Biblioteca de series"):
    with st.container(border=False,horizontal=True):
        ends:str=last_labour_day()
        starts:str=n_years_date(day=end, n=3)
        dct_swaps_id:dict={
        ...
        if st.session_state.categoria == "Tasas Swaps":
            ...
            dictionary = dct_swaps_id
        elif st.session_state.categoria == "TPM":
            lst_options:list=dct_tpm_id.keys()
            dictionary = dct_tpm_id
        st.multiselect("Seleccione Serie",options=lst_options,key="diccionario")
        st.date_input("Fecha De Inicio", key="starts", format="YYYY-MM-DD", value=starts)
        st.date_input("Fecha De Termino", key="ends", format="YYYY-MM-DD", value=ends)
    if st.session_state.diccionario == []:
        tab1, tab2 = st.tabs(["Grafico", "Data"])
        with tab1:
            st.text("Empty")
        tab2.dataframe()
    elif st.session_state.diccionario != None:
        ids = [dictionary[keys] for keys in st.session_state.diccionario]
        df_multibuscador = buscador_multiseries(id_serie = ids,
                                                starts=st.session_state.starts,
                                                ends=st.session_state.ends,
                                                nombre=st.session_state.diccionario
        )
        tab1, tab2 = st.tabs(["Grafico", "Data"])
        tab1.line_chart(df_multibuscador)
        tab2.dataframe(df_multibuscador)
